...
public abstract class AbstractCache<K, V> {
    ...
    public boolean put(K key, V value) {
        if (LOCAL_LOGV) {
            Log2.v(TAG, "Trying to put " + key + " into cache.");
        }
        ...
        if (mCacheMap.size() >= MAX_CACHED_ITEMS) {
            ...
            if (LOCAL_LOGV) {
                Log2.v(TAG, "Failed! size limitation reached.");
            }
            return false;
        }
        ...
        if (key != null) {
            ...
            if (LOCAL_LOGV) {
                Log2.v(TAG, key + " cached, " + mCacheMap.size() + " items total.");
            }
            return true;
        }
        return false;
    ...
    public V get(K key) {
        if (LOCAL_LOGV) {
            Log2.v(TAG, "Trying to get " + key + " from cache.");
        }
        ...
        if (key != null) {
            ...
            if (cacheEntry != null) {
                cacheEntry.hit++;
                if (LOCAL_LOGV) {
                    Log2.v(TAG, key + " hit " + cacheEntry.hit + " times.");
                }
    ...
    public V purge(K key) {
        if (LOCAL_LOGV) {
            Log2.v(TAG, "Trying to purge " + key);
        }

        CacheEntry<V> v = mCacheMap.remove(key);

        if (LOCAL_LOGV) {
            Log2.v(TAG, mCacheMap.size() + " items cached.");
        }
    ...
    public void purgeAll() {
        if (LOCAL_LOGV) {
            Log2.v(TAG, "Purging cache, " + mCacheMap.size()
                    + " items dropped.");
        }
        mCacheMap.clear();
    }
