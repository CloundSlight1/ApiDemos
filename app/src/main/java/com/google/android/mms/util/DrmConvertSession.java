        ...
        if (context != null && mimeType != null && !mimeType.equals("")) {
            try {
                ...
                try {
                    ...
                    Log2.w(TAG, "Conversion of Mimetype: " + mimeType
                            + " is not supported.", e);
                } catch (IllegalStateException e) {
                    Log2.w(TAG, "Could not access Open DrmFramework.", e);
                }
            } catch (IllegalArgumentException e) {
                Log2.w(TAG,
                        "DrmManagerClient instance could not be created, context is Illegal.");
            } catch (IllegalStateException e) {
                Log2.w(TAG, "DrmManagerClient didn't initialize properly.");
            }
        }
    ...
    public byte[] convert(byte[] inBuffer, int size) {
        ...
        if (inBuffer != null) {
            ...
            try {
                ...
                Log2.w(TAG, "Buffer with data to convert is illegal. Convertsession: "
                        + mConvertSessionId, e);
            } catch (IllegalStateException e) {
                Log2.w(TAG, "Could not convert data. Convertsession: " +
                        mConvertSessionId, e);
            }
        } else {
            ...
            try {
                ...
                if (convertedStatus == null ||
                    ...
                    try {
                        ...
                        Log2.w(TAG, "File: " + filename + " could not be found.", e);
                    } catch (IOException e) {
                        result = Downloads.Impl.STATUS_FILE_ERROR;
                        Log2.w(TAG, "Could not access File: " + filename + " .", e);
                    } catch (IllegalArgumentException e) {
                        result = Downloads.Impl.STATUS_FILE_ERROR;
                        Log2.w(TAG, "Could not open file in mode: rw", e);
                    } catch (SecurityException e) {
                        Log2.w(TAG, "Access to File: " + filename +
                        ...
                        if (rndAccessFile != null) {
                            try {
                                ...
                                result = Downloads.Impl.STATUS_FILE_ERROR;
                                Log2.w(TAG, "Failed to close File:" + filename
                                        + ".", e);
                            }
                        }
                    }
                ...
                Log2.w(TAG, "Could not close convertsession. Convertsession: " +
                        mConvertSessionId, e);
            }
        }
        return result;
