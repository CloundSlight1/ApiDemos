...
//                   //Xlog.e(LOG_TAG, "relative token");
    ...
    private int makeMessageBody(int pduType) {
        ...
        if (null == body || body.getPartsNum() == 0) {
            Log2.d(LOG_TAG, "makeMessageBody body == null");
            // empty message
...
//           //Xlog.i(LOG_TAG, "checkAddressType PDU_UNKNOWN_ADDRESS_TYPE");
    ...
    private int makeNotifyInd() {
        ...
        if (contentLocation != null) {
            Log2.d(LOG_TAG, "makeNotifyInd contentLocation != null");
            appendTextString(contentLocation);
        } else {
            Log2.d(LOG_TAG, "makeNotifyInd contentLocation  = null");
        }
        ...
        if (subject != null) {
            Log2.d(LOG_TAG, "makeNotifyInd subject != null");
    ...
    private int makeRetrievePdu() {
        Log2.d(LOG_TAG, "makeRetrievePdu begin");
        if (mMessage == null) {
            mMessage = new ByteArrayOutputStream();
            mPosition = 0;
        }
        ...
        if (trid == null) {
            // Transaction-ID should be set(by Transaction) before make().
            Log2.d(LOG_TAG, "Transaction ID is null");
        } else {
            // X-Mms-Transaction-ID
            appendOctet(PduHeaders.TRANSACTION_ID);
            appendTextString(trid);
        ...
        //  Message body
        makeMessageBody(1);
        Log2.d(LOG_TAG, "makeRetrievePdu end");
        return PDU_COMPOSE_SUCCESS;  // Composing the message is OK
    }
    /// @}
}
